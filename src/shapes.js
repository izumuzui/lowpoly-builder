...
export function appendRings(part, dimensions, boneIndex, buffers, detail = DETAIL_LEVELS.normal) {
  ...
  for (let r = 0; r < rings.length - 1; r += 1) {
    for (let i = 0; i < sides; i += 1) {
      ...
      const omitThreshold = part.omitFacingThreshold
      const isNarrowOpening = facing === basis.bPos && omitThreshold != null
      const shouldOmit = part.omitFacings?.includes(facing)
        && (!isNarrowOpening || Math.cos(i * step) > omitThreshold)
